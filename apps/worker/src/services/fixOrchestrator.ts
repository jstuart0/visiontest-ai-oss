...
export class FixOrchestrator {
  ...
  private async generateSuggestedActions(
    ...
    if (classification.classification === 'PRODUCT_BUG') {
      actions.push({
        ...
        description: 'Let VisionTest investigate the codebase and propose a fix',
  ...
  private generateFinalSummary(
    ...
    return {
      ...
      plainLanguage: `${analysis.plainLanguage} VisionTest has generated a fix that modifies ${patch.files.length} file(s). ${verification.outcome === 'passed' ? 'All verification checks passed.' : 'Some verification checks did not pass — manual review recommended.'}`,
